...
/*
 *  Insertion of ready queue at the head
 */
Inline void
...
/*
 *  Insertion at the end of the ready queue
 */
Inline void
...
/*
 *  Delete first task of ready queue
 */
Inline TaskType
...
/*
 * Bitmap of the priority of the task in the ready queue
 *
 * Set to 0 when the ready queue is empty (when there is no task in the executable state).
 */
 ...
 * Activate task
 *
 * Be aware that make_active may be called for the invoking task
 * in TerminateTask or ChainTask.
 */
BOOL
...
make_runnable(TaskType tskid)
{
    ...
    if (schedtsk != TSKID_NULL) {
        ...
        if (pri <= schedpri) {
            /*
             * When schedtsk has higher priority，
             * place tskid at the end of the ready queue.
             */
        ...
        /*
         * If tskid has higher priority, put schedtsk at the head of ready queue
         * and tskid as new schedtsk.
         */
